export class Client {
  ...
  async getHistoryEvent(id) {
	const historyEvent = await fetch(`/api/hisory/${id}`);
	return historyEvent.json();
  }
   ...
   * */
  async getRockets() {
	const rocketsInfo = await fetch('/api/rockets');
	return rocketsInfo.json();
  }
   ...
   * */
  async getRocket(id) {
	const rocket = await fetch(`/api/rockets/${id}`);
	return rocket.json();
  }
   ...
   * */
  async getRoadster() {
	const roadster = await fetch(`/api/roadster`);
	return roadster.json();
  }
